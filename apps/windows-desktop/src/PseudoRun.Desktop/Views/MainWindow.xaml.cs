...
namespace PseudoRun.Desktop.Views
{
    public partial class MainWindow : Window
    {
        ...
        private IServiceProvider Services => ((App)Application.Current).Services;
        ...
        {
            var dialog = Services.GetRequiredService<ExamplesDialog>();
            dialog.Owner = this;
        ...
        {
            var dialog = Services.GetRequiredService<TutorialDialog>();
            dialog.Owner = this;

            if (dialog.ShowDialog() == true && !string.IsNullOrEmpty(dialog.SelectedCode)
                && DataContext is MainViewModel viewModel)
            {
                viewModel.Code = dialog.SelectedCode;
            }
        }

        private void PracticeProblems_Click(object sender, RoutedEventArgs e)
        {
            var dialog = Services.GetRequiredService<PracticeProblemsDialog>();
            dialog.Owner = this;

            if (dialog.ShowDialog() == true && !string.IsNullOrEmpty(dialog.SelectedSolution)
                && DataContext is MainViewModel viewModel)
            {
                viewModel.Code = dialog.SelectedSolution;
            }
        }

        private void SyntaxReference_Click(object sender, RoutedEventArgs e)
        {
            var dialog = Services.GetRequiredService<SyntaxReferenceDialog>();
            dialog.Owner = this;
            dialog.ShowDialog();
        }
        ...
        {
            var dialog = Services.GetRequiredService<ExamModeDialog>();
            dialog.Owner = this;
            dialog.ShowDialog();
            // If exam was started, could disable certain menu items here
            // using dialog.ExamWasStarted. For now, just showing is sufficient.
        }
